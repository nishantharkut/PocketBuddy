...
function PoolDetail() {
  ...
  const grouped = ((items ?? []) as Item[]).reduce<Record<string, Item[]>>((acc, i: any) => {
    ...
    if (navigator.share) {
      try {
        await navigator.share({
          ...
          text: `Join my ${pool.platform} pool on PocketBuddy!`,
          url,
        });
        return;
      } catch {
  ...
  return (
    <AppShell>
      <div className="sticky top-0 z-30 flex h-14 items-center gap-3 border-b border-border bg-[color:var(--surface)] px-4">
        <button onClick={() => nav({ to: "/pool" })} className="text-muted-foreground">
        ...
        <Card id="card-split-summary" className="bg-[color:var(--surface-raised)] p-3">
          ...
          <div className="mt-2 space-y-1">
            {people.map((p) => {
              ...
              return (
                <p key={p} className="text-[13px] tnum">
                  {p}: {rupees(itemsTotal + deliveryPerPerson)}
                </p>
              );
            })}
            {people.length === 0 && (
          ...
          <p className="text-[12px] text-muted-foreground tnum">
            Delivery {rupees(pool.delivery_fee)} ÷ {people.length || 1} people ={" "}
            {rupees(deliveryPerPerson)} each
          </p>
        </Card>
        ...
        <div id="list-pool-items">
          {Object.entries(grouped).map(([who, its]) => (
            <div key={who} className="mt-3">
              <div className="text-[11px] font-semibold text-muted-foreground tracking-wide border-b border-border pb-1">
                {who}
              </div>
              {its.map((it) => (
      ...
      >
        {expired ? (
          <div className="rounded bg-[color:var(--pb-amber)]/20 p-2 text-center text-[12px] text-[color:var(--pb-amber)]">
            This pool is closed
          </div>
        ) : (
          <>
